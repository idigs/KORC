...
SUBROUTINE sample_distribution(params,g,eta,go,etao)
	...
	if (params%mpi_params%rank.EQ.0_idef) then
		CALL MPI_SCATTER(go,1,MPI_REAL8,MPI_IN_PLACE,1,MPI_REAL8,0,MPI_COMM_WORLD,mpierr)
		CALL MPI_SCATTER(etao,1,MPI_REAL8,MPI_IN_PLACE,1,MPI_REAL8,0,MPI_COMM_WORLD,mpierr)
	else
		CALL MPI_SCATTER(MPI_PROC_NULL,1,MPI_REAL8,go,1,MPI_REAL8,0,MPI_COMM_WORLD,mpierr)
		CALL MPI_SCATTER(MPI_PROC_NULL,1,MPI_REAL8,etao,1,MPI_REAL8,0,MPI_COMM_WORLD,mpierr)
	end if
