...
!! @note KORC and its documentation are in constant evolution. This makes it possible that there are issues that need to be solved. If you find an issue please report it immediatly through the "issues" section of the github repository.
!!
...
!!
!! @subsection s1s2 Getting started
...
!! <li>The <a href="https://w3.pppl.gov/ntcc/PSPLINE/">PSPLINES</a> library for interpolating the pre-computed electric and magnetic fields as well as the plasma profiles.</li>
...
!!
!! If you are installing the HDF5 and PSPLINES libraries in your system, we recommend to perform a local installation. Then enter the absolute path of the folder containing the <em>"lib"</em> and <em>"bin"</em> folders of each library to the PSPLINE_INSTALL and HDF5_INSTALL variables of the Makefile.
!!
!!
!! @note PSPLINES is known for having precision issues sometimes when compiled using the INTEL compilers suite in Linux systems. Please, double-check that the interpolations are giving the correct numbers.
!!
!! @subsection s2s2 Cloning and compiling KORC
...
!! <li>Type the following in the terminal <em>"git https://github.com/ORNL-Fusion/KORC.git"</em>. This will copy all the KORC files from the Github repository to your local system.
