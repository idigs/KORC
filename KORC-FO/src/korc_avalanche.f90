...
!	write(6,'("Time elapsed: ",F15.10)') time_elapsed
	...
	if (params%mpi_params%rank.EQ.0_idef) then
		...
		eta_buffer = aval_params%min_pitch_angle + (aval_params%max_pitch_angle - aval_params%min_pitch_angle)*rand_unif
		...
		do while(num_accepted.LT.nsamples)
			...
			do while ( (ii.LT.nsamples).AND.(num_accepted.LT.nsamples) )
				lp = (p_tmp(ii).LE.aval_params%max_p).AND.(p_tmp(ii).GE.aval_params%min_p)
				leta = (eta_tmp(ii).LE.aval_params%max_pitch_angle).AND.(eta_tmp(ii).GE.aval_params%min_pitch_angle)
				if (lp.AND.leta) then
