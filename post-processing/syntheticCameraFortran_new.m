function ST = syntheticCameraFortran_new(path,lambdas,visible,range)
...
if (ST.params.synthetic_camera_params.integrated_opt == 1)
    list={'Psyn_angular_pixel','np_angular_pixel','Psyn_lambda_pixel','np_lambda_pixel','Psyn_p_pplane','Psyn_t_pplane','np_p_pplane','np_t_pplane',...
        'PTot_p_pplane','PTot_t_pplane','P_p_lambda','P_t_lambda','np_p_lambda','np_t_lambda','P_a_pixel','P_l_pixel','np_pixel'};
else
    list={'PTot_pplane'};
end
...
for ll=1:length(list)
    ...
    for ss=1:ST.params.simulation.num_species
        try
            if (strcmp(list{ll},'P_p_lambda') || strcmp(list{ll},'P_t_lambda') || strcmp(list{ll},'P_a_pixel') || strcmp(list{ll},'P_l_pixel'))
            ...
            elseif ( strcmp(list{ll},'np_p_lambda') || strcmp(list{ll},'np_t_lambda') )
                if (ST.params.synthetic_camera_params.toroidal_sections == 1)
            ...
            elseif (strcmp(list{ll},'Psyn_p_pplane') || strcmp(list{ll},'np_p_pplane') || strcmp(list{ll},'PTot_p_pplane') ||...
        ...
        for ii=1:NR
            for jj=1:NZ
                if numel(size(ST.data.(['sp' num2str(ss)]).Psyn_pplane)) == 4
                    ...
                    Psyn_p_L1(ii,jj) = sum(abs(ST.data.(['sp' num2str(ss)]).PTot_p_pplane(ii,jj,:)),3);
                    Psyn_t_L1(ii,jj) = sum(abs(ST.data.(['sp' num2str(ss)]).PTot_t_pplane(ii,jj,:)),3);
                else
                    ...
                    Psyn_p_L1(ii,jj) = abs(ST.data.(['sp' num2str(ss)]).PTot_p_pplane(ii,jj));
                    Psyn_t_L1(ii,jj) = abs(ST.data.(['sp' num2str(ss)]).PTot_t_pplane(ii,jj));
                end
...
%     yAxis_rescaled = yAxis;
    ...
    if (NT ~= 0)
        ...
        for tt=1:double(NT)
            ...
            if ST.params.synthetic_camera_params.photon_count
                xlabel(hc,'$\mathcal{P}_{Tot}$ (photons)','Interpreter','latex','FontSize',12)
            else
                xlabel(hc,'$\mathcal{P}_{Tot}$ (Watts)','Interpreter','latex','FontSize',12)
            end
        ...
        if plotToroidalSections
            for tt=1:NT
                ...
                if (~isempty(B3) && ~isempty(B4))
                    ...
                    ax = gca;ax.Color = [1,1,1];ax.ClippingStyle = 'rectangle';
                    if ST.params.synthetic_camera_params.photon_count
                        xlabel(hc,'$\int \mathcal{P}_R(\lambda) d\lambda$ (photons)','Interpreter','latex','FontSize',12)
                    else
                        xlabel(hc,'$\int \mathcal{P}_R(\lambda) d\lambda$ (Watts)','Interpreter','latex','FontSize',12)
                    end
